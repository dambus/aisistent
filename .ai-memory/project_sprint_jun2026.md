...
Brainstorm D1 implementiran: JSON-LD (BreadcrumbList+HowTo+FAQPage) + vidljiva FAQ sekcija + keyword interno linkovanje ka kalkulatorima na `/obrasci/[slug]`, sve bez nove DB kolone. Testirano uživo. Upload & Fill potpuno ukinut na Milanov zahtev (previše komplikovano/nepouzdano) — mrtav kod obrisan, `curate-form.ts`/biblioteka obrazaca netaknuti. Detalji: `PROGRESS.md` (10. jul, četvrti dodatak). **Potvrđeno na produkciji** (aisistent.rs) posle redeploy-a — FAQ sekcija i JSON-LD live.

**Vercel deploy gotcha (10. jul):** prvi deploy ovog commit-a je pukao bez ijedne linije greške u logu — build je stao odmah posle "Cloning completed", ništa dalje. Uzrok: `playwright` (devDependency, koristi je samo `scripts/harvest-sources.ts` lokalno, nigde se ne importuje u `app/`/`lib/`) ima `postinstall` hook koji preuzima Chromium (~300MB); kad Vercel build cache promaši (cache miss na `npm install`), taj download može da visi/predugo traje u build sandbox-u i Vercel tiho ubije build bez jasne poruke. **Fix: env var `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` dodat u Vercel dashboard (Project Settings → Environment Variables, sva okruženja)** — posle toga redeploy je prošao čisto. Nije potreban commit u repo (Vercel-only podešavanje). Ako se build ikad opet tiho zaustavi posle clone-a bez greške — prvo proveriti da li je ovaj env var i dalje podešen (npr. posle promene Vercel projekta ili novog tima).
