...
| BUG-052 (produkcioni sadržaj) | ova sesija | Isti "šest padeža" bug kao BUG-051, drugi fajl: `docs/marketing/social-content-pack-2026-07-13.html:442` — nije uhvaćeno u prvom prolazu (3. avgust) jer je proverena samo Instagram grana, ne stariji content pack. Otkriveno primenom Zadatka 3 checklist-e retroaktivno na sav `docs/marketing/`. Ispravljeno. |
| BUG-053 (produkcioni sadržaj) | ova sesija | `content/blog/ugovor-o-delu-vs-ugovor-o-radu.md:48` tvrdio "Zakon o radu u članu 31." za princip prekvalifikacije prikrivenog radnog odnosa — pogrešan broj člana (čl. 31 je o obavezi upoznavanja novog zaposlenog sa procesom rada, nevezano). Broj člana uklonjen, princip (tačan) ostaje bez lažne preciznosti. |
| ZADATAK-3 (rešeno) | ova sesija | `docs/serbian-language-facts.md` — referentni dokument sa proverenim jezičkim/zakonskim faktima + checklist pre objave javnog sadržaja, primenjen retroaktivno (uhvatio BUG-052, BUG-053). Skill za automatsko skeniranje (opcioni deo Zadatka 3) nije rađen. |
| BUG-054 (QA korak, rešeno) | ova sesija | Milan uhvatio uživo (light mode, `poslovni-mejl`, "zahvalnica za poslovnu saradnju"): QA korak (`lib/qa/review.ts`) je SAM napravio pravopisnu grešku umesto da je ispravi — promenio ispravno "Poštovani," (zarez) u pogrešno "Poštovani." (tačka). U srpskom pravopisu iza formule oslovljavanja u poslovnom pismu ide zarez, ne tačka. Fix: dodato eksplicitno pravilo u QA system prompt ("POZNATA PRAVILA — ne nagađaj suprotno") + generalna instrukcija "ako nisi 100% siguran, ne menjaj". Pokazuje da QA "fixed" lista nije garantovano tačna — model može uvesti grešku istom klasom nesigurnosti koju feature pokušava da otkloni. Nema automatski regresioni test za ovu klasu grešaka (za razliku od deklinacije) — razmotriti fixture test po istom obrascu ako se nađe još ovakvih slučajeva. |
