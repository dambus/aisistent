...
| `docs/serbian-language-facts.md` | Provereni jezički/zakonski fakti + checklist pre objave javnog sadržaja | Pre pisanja/objave bilo kog marketing/blog teksta koji tvrdi nešto o jeziku ili zakonu | Kad se nađe nova činjenica koja treba proveru, ili nova regresija (dodaj incident) |
| `lib/qa/pravila.ts` | Priručnik gramatičkih pravila koje QA korak (`lib/qa/review.ts`) mora da poštuje — raste kad QA sam napravi grešku (ne samo propusti tuđu) | Automatski ubačeno u QA system prompt na svakom pozivu — ne čita se ručno | Kad Milan uhvati QA korak da je napravio konkretnu jezičku grešku — dodaj red u `GRAMATICKA_PRAVILA`, uz BUG broj kao trag |
