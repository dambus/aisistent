...
**Status (8. jul 2026.): Faza 4 biblioteka obrazaca NA PRODUKCIJI, 73 obrasca.** Javna `/obrasci` biblioteka (APR privredna društva + preduzetnici: izvodi, prijave promena, rezervacija naziva, ispravke, potvrde, prepisi rešenja, svi Dodaci, JRPPS osnivačke forme; CROSO: ovlašćenje pravno lice), download popunjeno (Starter+) / prazno (javno), editabilan PDF bez flatten. Kuratorski CLI + harvester (APR privredna društva i preduzetnici: izvori iscrpljeni, svi AcroForm kandidati kurirani; CROSO: 1 kandidat, kuriran; PIO dodat kao izvor ali 0 AcroForm kandidata — sve flat, čeka konverziju; RFZO izvor probran pa izbačen — samo medicinski obrasci, nisu za naše korisnike). Upload & Fill uklonjen iz UI, pipeline = kuratorski alat. "Obrazac je zastareo?" feedback dugme live. Homepage promo baner + nav link + dashboard sidebar bedž — biblioteka se aktivno reklamira kao feature. Spec: `docs/obrasci/FAZA4_BIBLIOTEKA_OBRAZACA.md`.

**Sledeće za biblioteku:** poslednja 2 flat kandidata iz apr-privredna-drustva (van batch-curate obuhvata), novi izvor Poreska uprava (3 podstranice, pažnja na e-only obrasce), n8n cron za harvester, flat→AcroForm konverzija (otključava PIO-ove 18 M-obrazaca + preostale flat obrasce svuda).
